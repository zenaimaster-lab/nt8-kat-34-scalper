...
- Điều kiện môi trường — LONG (SHORT mirror ngược lại), **mỗi điều kiện có toggle riêng**:
  ...
  4. `Cond: EMA 34 slope angle` — góc nghiêng EMA 34 tối thiểu `+Min Angle` độ (lên) cho LONG; tối thiểu `-Min Angle` độ (xuống) cho SHORT. Góc chuẩn hoá tự động bằng ATR trên series 30s: `atan(Δema34/bar / ATR(period))` — 45° = slope 1 ATR/bar, không phụ thuộc zoom, không cần chỉnh tay theo instrument (`ATR Period` chỉnh được). **Default OFF** — slope bar 30s thường quá nhỏ so với ATR nên 30° hiếm khi đạt; bật tay khi muốn dùng.
